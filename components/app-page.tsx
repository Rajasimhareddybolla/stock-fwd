...
export function BlockPage() {
  ...
  const [historicalData, setHistoricalData] = useState(generateHistoricalData(selectedStock.symbol))
  ...
  const handleSaveStock = (symbol: string, price: number) => {
    ...
    // toast({
    //   title: "Stock Price Saved",
    //   description: `${symbol} price (₹${price.toFixed(2)}) has been saved`,
    // })
  }
  ...
  return (
    <div className="container mx-auto p-4 bg-gradient-to-br from-background to-secondary/20 min-h-screen">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="lg:col-span-2 transition-all duration-300 hover:shadow-lg bg-card/50 backdrop-blur-sm">
          <CardHeader>
            ...
            <CardDescription>Click on a stock to view its chart</CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    if (active && payload && payload.length) {
                      return (
                        <div className="bg-background/80 p-4 border border-border rounded-md shadow-md backdrop-blur-sm">
                          ...
                          <p className="text-primary">Price: ₹{typeof payload[0].value === 'number' ? payload[0].value.toFixed(2) : payload[0].value}</p>
